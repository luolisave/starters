import { Component, OnInit } from '@angular/core';

import { DataService } from '../data.service'

@Component({
  selector: 'app-my-new-component',
  templateUrl: './my-new-component.component.html',
  styleUrls: ['./my-new-component.component.css']
})
export class MyNewComponentComponent implements OnInit {

  myObj = {a:"aaa"};
  myArray = ["001", "002", "003"];

  angularLogo="https://www.primefaces.org/wp-content/uploads/2016/10/primeng.png";
  ImgBase64 = "data:image/png;base64,";

  constructor(private DataService: DataService) { }

  ngOnInit() {
  }

  myString = "--- data not loaded yet. ---";

  myClick(){
    this.myString = this.DataService.getMyData();
    console.log("you clicked me! \n \n "+this.DataService.getMyData());

    console.log("===> subscribe obj = ", this.DataService.getAjaxData());
  }

  myGet(){
    this.DataService.getAjaxData().subscribe(function(result){
      console.log("===> result.json =", result.json());
    })
  }

  myPost(){
    this.DataService.postAjaxData({"aa":"bb"}).subscribe(function(result){
      console.log("===> result.json =", result.json());
    })
  }

  myLogin(){
    this.DataService.apiLogin({"username":"admin","password":"pass1234"}).subscribe(function(result){
      console.log("===> apiLogin() result.json =", result.json());
    })
  }



}
